...
Contributors:
    C. McNatt
%}
function [geo] = makePanel_diamond(frontTheta, backTheta, width, draft, Nx, Ny, Nz, varargin)

half = checkOptions({'Half'}, varargin);
...
if (half)
    ...
    for n = 1:Nz
        for m = 1:Ny
            ...
            verts(1,:) = [x(n,1) y(m+1) z(n)]; % top right
            verts(2,:) = [x(n,1) y(m) z(n)]; % top left
            verts(3,:) = [x(n+1,1) y(m) z(n+1)]; % bottom left
            verts(4,:) = [x(n+1,1) y(m+1) z(n+1)]; % bottom right
    ...
    for n = 1:Nz
        for m = Nx:-1:1
            ...
            verts(1,:) = [x(n, m) y(end) z(n)]; % top right
            verts(2,:) = [x(n, m+1) y(end) z(n)]; % top left
            verts(3,:) = [x(n+1, m+1) y(end) z(n+1)]; % bottom left
            verts(4,:) = [x(n+1, m) y(end) z(n+1)]; % bottom right
    ...
    for n = 1:Nz
        for m = Ny:-1:1
            verts = zeros(4,3);
            verts(1,:) = [x(n,end) y(m) z(n)]; % top right
            verts(2,:) = [x(n,end) y(m+1) z(n)]; % top left
            verts(3,:) = [x(n+1,end) y(m+1) z(n+1)]; % bottom left
            verts(4,:) = [x(n+1,end) y(m) z(n+1)]; % bottom right
    ...
    for n = 1:Nz
        for m = 1:Nx
            verts = zeros(4,3);
            verts(1,:) = [x(n, m+1) y(1) z(n)]; % top right
            verts(2,:) = [x(n, m) y(1) z(n)]; % top left
            verts(3,:) = [x(n+1, m) y(1) z(n+1)]; % bottom left
            verts(4,:) = [x(n+1, m+1) y(1) z(n+1)]; % bottom right
    ...
    for n = Nx:-1:1
        for m = 1:Ny
            verts = zeros(4,3);
            verts(1,:) = [x(end, n+1) y(m+1) z(end)]; % top right
            verts(2,:) = [x(end, n+1) y(m) z(end)]; % top left
            verts(3,:) = [x(end, n) y(m) z(end)]; % bottom left
            verts(4,:) = [x(end, n) y(m+1) z(end)]; % bottom right
