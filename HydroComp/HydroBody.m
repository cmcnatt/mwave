...
classdef HydroBody < FloatingBody
    % A floating body with attached frequency domain hydrodynamic forces - added mass, 
    % damping, excitation force, a diffraction transfer matrix, a force 
    % excitation matrix etc. 
    %
    % Shall be used to compute hydrodnamic freq domain array interactions forces.
        ...
        function [hb] = HydroBody(body, varargin)
            ...
            if (isa(body, 'HydroBody'))
                ...
                if (~isa(hydF, 'FreqDomForces'))
                    error('hydF must be a FreqDomForces object');
                end
